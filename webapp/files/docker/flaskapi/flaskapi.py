...
def add_user():
    ...
    if name and email and request.method == "POST":
        ...
        try:
            ...
            cursor.close()
            conn.close()
            resp = jsonify(f"User created successfully!")
            resp.status_code = 200
            return resp
        except Exception as exception:
            return jsonify(str(f"{exception}{app.config['MYSQL_DATABASE_HOST']}{app.config['MYSQL_DATABASE_PASSWORD']}{app.config['MYSQL_DATABASE_USER']}"))
    else:
...
def update_user():
    ...
    if name and email and user_id and request.method == "POST":
        # save edits
